...
ServerEvents.recipes(event => {
	...
	let addRecipes = function(event) {
		...
		if (isMetal && Fox.Processing.UsingReducedSmelting) {
			// Add Smelting
			namespace.Crushing.AddRecipe(event, data.raw, data.nugget, namespace.GetSmeltingAmount());
			// Add Blasting
			namespace.Crushing.AddRecipe(event, data.raw, data.nugget, namespace.GetBlastingAmount());
			// Millstone
			namespace.Millstone.AddRecipe(event, data.raw, data.crushed, 1);
		}
